...
import pytest
from sgl_kernel import (scaled_fp4_grouped_quant, flashinfer_cutedsl_moe_masked, flashinfer_cutedsl_grouped_gemm_nt_masked)
...
def test_grouped_gemm_nt_masked(
    ...
    torch.testing.assert_close(out_flashinfer.permute(2,0,1).cpu(), out_ref.cpu(), atol=1e-1, rtol=1e-1)
...
def test_moe_masked(
    ...
    device="cuda"
    B, D, N, num_experts, topk = shape
    hidden_states = torch.randn(B, D, dtype=torch.bfloat16, device=device) / 5.
    w1 = torch.randn(num_experts, 2 * N, D, dtype=torch.bfloat16, device=device) / 10.
    w2 = torch.randn(num_experts, D, N, dtype=torch.bfloat16, device=device) / 10.
    router_logits = torch.randn(B, num_experts, dtype=torch.float32)
    ...
    a2_global_scale = torch.ones((num_experts,), dtype=torch.float32, device=hidden_states.device) # assume intermediate scale is 1.0
    ...
    ref_output = torch_moe_nvfp4(
        a_in_dtype, w1_d, w2_d, topk, routing_weights.to(a_in_dtype.device), 
    ...
    torch.testing.assert_close(out_weighted.cpu(), ref_output.cpu(), atol=1e-1, rtol=1e-1)
