...
void launch_sm100_fp8_group_mm(
  ...
  using CollectiveEpilogue = typename cutlass::epilogue::collective::CollectiveBuilder<
    ...
    typename ScheduleConfig::MmaTileShape, typename ScheduleConfig::ClusterShape, // can be specified at runtim l:123
    cutlass::epilogue::collective::EpilogueTileAuto,
  ...
  using CollectiveMainloop = typename cutlass::gemm::collective::CollectiveBuilder<
    ...
    ElementAccumulator,
    typename ScheduleConfig::MmaTileShape, typename ScheduleConfig::ClusterShape,
  ...
  auto can_implement_status = gemm_op.can_implement(arguments);
...
void sm100_fp8_group_mm_dispatch_shape(
    torch::Tensor& output,
...
struct MMA2SMConfig {
  ...
  torch::Tensor workspace = torch::empty(1000, options_int);
  ...
  // if (a.size(0) <= 512) {
    run_get_group_gemm_starts(
        ...
        false);
    launch_sm100_fp8_group_mm<OutType, MMA1SMConfig, cutlass::layout::ColumnMajor>(
        out_ptrs,
        a_ptrs,
        b_ptrs,
        stride_a,
...
void fp8_grouped_mm(
  ...
  if (sm_version == 100) {
    if (output.scalar_type() == torch::kBFloat16) {
      sm100_fp8_group_mm_dispatch_shape<cutlass::bfloat16_t>(
          output,
          a,
          b,
          stride_a,
      ...
      sm100_fp8_group_mm_dispatch_shape<cutlass::half_t>(
          output,
          a,
          b,
          stride_a,
  ...
  TORCH_CHECK_NOT_IMPLEMENTED(
      can_implement, "No implemented fp8_scaled_mm for current compute capability: ", sm_version);
}
