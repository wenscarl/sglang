...
def flashinfer_cutedsl_moe_masked(
    ...
    down_input = torch.empty(
        *down_input_shape, dtype=gateup_output.dtype, device=gateup_output.device
    )
    silu_and_mul(gateup_output, down_input)

    down_input = down_input.view(num_experts, m, n)  # [l, m, n * 2]

    # Quantize intermediate tensor
    # TODO(shuw): silu_and_mul + scaled_fp4_grouped_quant can be fused in later
    # release.
