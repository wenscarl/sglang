...
def modelopt_config_to_compressed_tensors_config(
    ...
    logger.info(
        "ModelOpt->Compressed Tensors bridge: using CompressedTensorsW8A8Fp8 (channel, dynamic per-token) for %s",
        ModelOptQuantizationScheme.FP8_PER_CHANNEL_PER_TOKEN_CFG.value,
    )
...
def modelopt_config_to_compressed_tensors_config_block(
    ...
    logger.info(
        "ModelOpt->Compressed Tensors bridge: using FP8 block (linear_fp8_config) weight_block_size=%s for %s",
        weight_block_size,
